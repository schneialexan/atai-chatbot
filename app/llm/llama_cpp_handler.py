...
class LlamaCppHandler:
    """A handler for a single LLM or multimodal model using llama-cpp-python."""
    ...
    def __init__(self, model_repo: str, model_file: str = None, model_type: str = 'llm', 
                 ...
                 n_gpu_layers: int = 0, n_ctx: int = 4096, auto_load: bool = True, **kwargs):
        print(f"Initializing LlamaCppHandler for model: {model_repo}/{model_file}")
    ...
    def _download_hf_file(self, repo, filename):
        if not filename:
            return None
    ...
    def _load_model(self):
        ...
        self.model_path = self._download_hf_file(self.model_repo, self.model_file)
        chat_handler = None
        if self.model_type == 'multimodal':
            if not self.mmproj_file:
                raise ValueError("'mmproj_file' is required for multimodal models.")
            self.mmproj_path = self._download_hf_file(self.mmproj_repo, self.mmproj_file)
